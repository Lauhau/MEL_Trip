...
const ExpensesView: React.FC<ExpensesViewProps> = ({ expenses, setExpenses, categories, setCategories, isReadOnly = false }) => {
  // Expense Form State
  ...
  const addExpense = () => {
    if (!title || (!inputAud && !inputTwd) || isReadOnly) return;
  ...
  return (
    <div className="p-4 pb-28 h-full overflow-y-auto bg-surface dark:bg-darkSurface transition-colors no-scrollbar">
      ...
      {/* Add Expense Form (Hide if Read Only) */}
      {!isReadOnly && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-5 mb-6 border border-gray-100 dark:border-none">
