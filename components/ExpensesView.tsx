...
const ExpensesView: React.FC<ExpensesViewProps> = ({ expenses, setExpenses }) => {
  ...
  return (
    <div className="p-4 pb-28 h-full overflow-y-auto bg-surface">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-5 mb-6 border border-gray-100">
        <h2 className="text-lg font-bold text-gray-800 mb-4">目前結算 (AUD)</h2>
        <div className="space-y-3">
          {USERS.map(user => {
            const bal = balances[user];
            return (
              <div key={user} className="flex justify-between items-center border-b border-gray-50 last:border-0 pb-2 last:pb-0">
                <span className="font-medium text-gray-600">{user}</span>
                <span className={`font-bold ${bal >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}>
        ...
        <div className="mt-4 pt-3 border-t border-dashed border-gray-200 text-xs text-gray-400 text-center">
            {balances[USERS[0]] > 0 
      ...
      {/* Add New Expense */}
      <div className="bg-white rounded-2xl shadow-sm p-5 mb-6 border border-gray-100">
        <h2 className="text-lg font-bold text-gray-800 mb-4">新增消費</h2>
        <div className="space-y-3">
          <input 
            className="w-full bg-gray-50 p-3 rounded-xl outline-none focus:ring-2 focus:ring-primary border-none"
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
                ...
                <input 
                    type="number" 
                    className="w-full bg-gray-50 p-3 pl-7 rounded-xl outline-none focus:ring-2 focus:ring-primary border-none"
                    placeholder="0.00"
                    value={amount}
            ...
            <select 
                className="bg-gray-50 p-3 rounded-xl outline-none focus:ring-2 focus:ring-primary border-none text-sm"
                value={payer}
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 my-2 overflow-x-auto">
            <span className="whitespace-nowrap">分攤者:</span>
            {USERS.map(u => (
                <button 
                    key={u}
                    onClick={() => toggleInvolved(u)}
                    className={`px-3 py-1 rounded-full border transition-colors whitespace-nowrap ${involved.includes(u) ? 'bg-primary text-white border-primary' : 'bg-white text-gray-400 border-gray-200'}`}
                >
                    {u}
                </button>
            ))}
          ...
          <button 
            ...
            className="w-full bg-accent text-white font-bold py-3 rounded-xl shadow-lg shadow-amber-200 disabled:opacity-50 hover:bg-amber-600 transition-colors"
          >
            記帳
          </button>
        </div>
      ...
      <div className="space-y-3">
        {expenses.map(exp => (
            <div key={exp.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-50 flex justify-between items-center">
                <div>
                    <div className="font-bold text-gray-800 text-sm">{exp.title}</div>
                    <div className="text-[10px] text-gray-400 mt-1">
                        <span className="text-primary font-medium">{exp.payer}</span> 支付 • {exp.involved.join(', ')} 分攤
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <div className="text-right">
                        <span className="font-bold text-gray-800 block">${exp.amount.toFixed(2)}</span>
